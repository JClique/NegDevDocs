<template>
	<div class="bg-[var(--vp-c-bg)] px-8 py-12 dark:bg-[var(--vp-c-bg-alt)] sm:py-20">
		<div
			v-if="title"
			class="mx-auto max-w-2xl lg:text-center"
		>
			<span class="mt-2 block text-pretty text-4xl font-semibold tracking-tight sm:text-5xl lg:text-balance">
				{{ title }}
			</span>
			<span
				v-if="description"
				class="mt-6 block text-pretty text-lg/8 text-[var(--vp-c-text-2)]"
				v-html="description"
			/>
		</div>
		<v-spacer
			...
			<dl class="grid max-w-xl grid-cols-1 gap-x-20 gap-y-10 lg:max-w-none lg:grid-cols-2 lg:gap-y-16">
				<div class="relative pl-16">
					<dt class="text-base/7 font-semibold">
						<div
							class="absolute left-0 top-0 flex size-10 items-center justify-center rounded-lg bg-red-800 p-2 text-white"
						>
							<svg
